...
def tf_cart (train_df:pd.DataFrame, valid_df:pd.DataFrame, test_df:pd.DataFrame):
    """
    Args:
    ...
    model = tfdf.keras.DistributedGradientBoostedTreesModel(verbose=2)
...
def main():
    ...
    model, output = tf_cart(train_df=train_df, valid_df=valid_df, test_df=test_df)

    plot_training_logs(model)

    output.to_csv("submissions/tf_cart.csv",index=False)
